...
def load_prepared_data():
    ...
    # LightGBM is more stable here with an explicit float matrix.
...
# MODEL
# =============================================================================
def build_lightgbm_model(num_classes: int) -> Any:
    return lgb.LGBMClassifier(
        # One-vs-all behaves much better than softmax on this 727-class problem.
...
def main() -> None:
    ...
    try:
        ...
        section_start = time.perf_counter()
        try:
            model = build_lightgbm_model(len(class_names))
        ...
        section_start = time.perf_counter()
        try:
            print("Training final LightGBM model on full training split...")
            model.fit(X_train, y_train)

            joblib.dump(model, ARTIFACTS_DIR / "lightgbm_model.pkl")
