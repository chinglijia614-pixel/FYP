...
def load_prepared_data():
    ...
    # Binary symptom matrices do not need int64; shrinking them reduces peak memory substantially.
    X_train = X_train.astype(FEATURE_DTYPE, copy=False)
    X_test = X_test.astype(FEATURE_DTYPE, copy=False)
    X_cv = X_cv.astype(FEATURE_DTYPE, copy=False)
...
# MODEL
# =============================================================================
def build_randomforest_model() -> Any:
...
def run_cross_validation(model: Any, X_cv: pd.DataFrame, y_cv: np.ndarray) -> pd.DataFrame:
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(skf.split(X_cv, y_cv), start=1):
        ...
        metrics = evaluate_model(fold_model, X_eval_fold, y_eval_fold)
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            print("=" * 72)
            print("TRAINING ENTRY - RANDOMFOREST")
            print("=" * 72)
            ...
            print(f"Feature dtype: {FEATURE_DTYPE.__name__}")
            print(f"RandomForest n_estimators: {RF_N_ESTIMATORS}")
            print(f"RandomForest max_depth: {RF_MAX_DEPTH}")
            print(f"RandomForest min_samples_leaf: {RF_MIN_SAMPLES_LEAF}")
            print(f"RandomForest max_samples: {RF_MAX_SAMPLES}")
        ...
        section_start = time.perf_counter()
        try:
            print("Training final RandomForest model on full training split...")
            model.fit(X_train, y_train)

            joblib.dump(model, ARTIFACTS_DIR / "randomforest_model.pkl")
